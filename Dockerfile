...
# --chmod: git and Linux build contexts keep the script's 0644 mode, and ECS
# runs it directly as the container command.
COPY --chmod=0755 scripts/with-config.sh /usr/local/bin/with-config
USER 10001
ENV LOG_LEVEL=info
ENTRYPOINT []
